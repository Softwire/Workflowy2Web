  ...
  var Outline = function (node, siteTitle, generator, title, fileName, filePath, parentNavigationObject) {
    ...
    function getChildPages() {
      ...
      $.each($(node).children(), function (index, child) {
        var childTitle = Util.stripText($(child).attr('text'));
        if (Util.isPage(childTitle)) {
          if (childTitle[0] == '[') {
            $.each(childTitle.substring(1, childTitle.length - 1).split(','), function(index, subTitle) {
              pages.push({ title: subTitle, fileName: Util.stripText(subTitle, true).toLowerCase(), node: child });
            });
          } else {
            pages.push({ title: childTitle, fileName: Util.stripText(childTitle, true).toLowerCase(), node: child });
          }
        }
      });
      return pages;
